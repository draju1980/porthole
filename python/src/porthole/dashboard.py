"""Self-contained HTML dashboard for the inspector."""
...
function showDetail(req) {
  ...
  $("detailReqHeaders").textContent = JSON.stringify(req.request_headers, null, 2);
  $("detailReqBody").textContent = req.request_body || "(empty)";
  $("detailRes").textContent = "Status: " + (req.status || "pending") + (req.duration != null ? "  (" + req.duration + "ms)" : "");
  $("detailResHeaders").textContent = req.response_headers ? JSON.stringify(req.response_headers, null, 2) : "(none)";
  $("detailResBody").textContent = req.response_body || "(empty)";
...
function updateStats(stats) {
  ...
  $("reqCount").textContent = stats.request_count;
}
